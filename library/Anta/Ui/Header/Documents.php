 ...
 *
 */
 class Anta_Ui_Header_Documents {
	...
	protected function _getFilters(){
		...
		if( !empty( $query ) ){
			...
			$content .= '<span class="is-untouchable-tag"><em>title</em><b>: '.$query.'</b><a href="'.$delLink.'" style="padding-left:6px"><img class="tag-icon" src="'.Anta_Core::getBase().'/images/cross-small.png"></a></span>';
		}
	...
	protected $_timeline = "33";
	
	public function setTimeline( Ui_D3_Timeline $timeline ){
	...
	public function __toString(){
		...
		$hasRightOffset = $offset +$limit < $this->totalItems;
		...
		<div class="grid_24 alpha omega item-header" style="padding-bottom:6px; vertical-align:middle">
		  ...
		  <div class="grid_18">
		    <div class="grid_18 margin_1 alpha omega">
		      <strong class="black">'.$this->totalItems.'</strong> documents ('.$offset." - ".min( $offset + $limit, $this->totalItems ).')
		      '.$this->_getFilters().'<button id="open-tag-panel">open tag panel</button>
		    </div>
		  ...
		  '.( $hasRightOffset === false ?'':
			// left offset
			'<a href="'.$_SERVER['REFERRER_URI'].'?'.Dnst_Filter::setProperty( 'offset', $this->totalItems - $limit ).'"
