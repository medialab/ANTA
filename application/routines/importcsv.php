...
if (($handle = fopen($source, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 0, ";")) !== FALSE) {
    	print_r($line_number == 0 ? $data : '');
    	
    	if($line_number == 0) {
    		$headers = $data;
    	} else {
    		$entry = (object) array_combine($headers, $data);
    		$entry->categories = explode('/', $entry->category);
    		if(store_document($_user, $entry) === false)
    			break;
    	}
...
print_r(array('lines'=>$line_number));

/*
	Take a line object and save it to anta.

*/
function store_document($_user, $entry) {
	try{
		$idDocument = Application_Model_DocumentsMapper::addDocument(
			...
			"text/plain", 
			'',
			Anta_Core::getDate( $entry->date, "Y/m/d" ),
			'fr',
			$entry->permalink
		);
	...
	foreach( $entry->categories as $l_cat){
